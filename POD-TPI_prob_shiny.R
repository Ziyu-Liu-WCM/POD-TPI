...
# # # #
# 
# df <- data.frame(dose = c(1, 2, 2, 2, 3, 3),
#                  event_time = c(28, 28, 9, 26, 15, 9),
#                  event = c(0, 0, 1, 1, 1, 1))
# 
# 
# 
# POD_TPI_df(df, W = 28, MaxDose = 5,
...
POD_TPI_decision_shiny <- function(W, MaxDose, 
  ...
  # Check if Dose levels are identical within pending patients
  ...
  if(r_d == 0){
    ...
    return(sprintf("There is no pending patient. Dose decision is made based on complete data and no probability will be calculated. Dose decision: %s. Dose assignment for this patient: %d. Current dose: %d, DLT Patients: %d, Non-DLT Patients: %d, Pending Patients: %d.",
    ...
    if(type_suspension == 1 & r_d > (n_d + m_d + r_d) / 2){
        ...
        return(sprintf("Patient SUSPEND(based on r_d > (n_d + m_d + r_d) / 2). Current Dose: %d, DLT Patients: %d, Non-DLT Patients: %d, Pending Patients: %d. No probability calculated", 
        ...
        return(sprintf("Patient SUSPEND(based on n_d + m_d = 0, no patient with observed outcome). Current Dose: %d, DLT Patients: %d, Non-DLT Patients: %d, Pending Patients: %d. No probability calculated", 
                    current_dose, n_d, m_d, r_d))
      ...
      if(length(unique(dose_decisions_s_d)) > 1) {
          ...
          return(sprintf("Patient SUSPEND(based on multiple possible decisions). Current dose: %d, DLT Patients: %d, Non-DLT Patients: %d, Pending Patients: %d. No probability calculated", 
                                       current_dose, n_d, m_d, r_d))
      } 
    }
    ...
    if(type_suspension == 3) {
      ...
      return(sprintf("Dose decision: %s. Dose assignment for this patient: %d. Current dose: %d, DLT Patients: %d, Non-DLT Patients: %d, Pending Patients: %d. Probability of De-escalation： %f, Probability of Stay: %f, Probability of Escalation, %f.",
                                     decision_vec[dose_decision+2], current_dose + dose_decision, current_dose, n_d, m_d, r_d, prob_dose_decisions[1], prob_dose_decisions[2], prob_dose_decisions[3]))
      ...
      if(dose_decisions[max_index] == 1){
        ...
        if( type_suspension == 2 & max_prob < q1 ){
          ...
          # SUSPEND
          return(sprintf("Patient SUSPEND(based on probability thresholds, probability of escalation < q1). Current dose: %d, DLT Patients: %d, Non-DLT Patients: %d, Pending Patients: %d. Probability of De-escalation： %f, Probability of Stay: %f, Probability of Escalation, %f.", 
                       current_dose, n_d, m_d, r_d, prob_dose_decisions[1], prob_dose_decisions[2], prob_dose_decisions[3]))
        ...
        if(type_suspension == 2 & prob_dose_decisions[1] > q2 & current_dose != 1){
          ...
          # SUSPEND
            return(sprintf("Patient SUSPEND(based on probability thresholds, probability of de-escalation > q2). Current dose: %d, DLT Patients: %d, Non-DLT Patients: %d, Pending Patients: %d. Probability of De-escalation： %f, Probability of Stay: %f, Probability of Escalation, %f.", 
                        current_dose, n_d, m_d, r_d, prob_dose_decisions[1], prob_dose_decisions[2], prob_dose_decisions[3]))
